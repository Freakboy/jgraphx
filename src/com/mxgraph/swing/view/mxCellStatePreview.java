...
public class mxCellStatePreview {
    ...
    protected void translateState(mxCellState parentState, mxCellState state,
        ...
        if (state != null) {
            ...
            if (model.isVertex(cell)) {
                ...
                graph.getView().validateBounds(state,cell);
            ...
            for (int i = 0; i < childCount; i++) {
                translateState(state, graph.getView()
    ...
    protected mxRectangle revalidateState(mxCellState parentState,
        ...
        if (state != null) {
            ...
            for (int i = 0; i < childCount; i++) {
                mxRectangle tmp = revalidateState(state,
                        graph.getView().getState(model.getChildAt(cell, i)), dx, dy);
    ...
    protected void paintPreviewState(mxGraphics2DCanvas canvas,
                                     mxCellState state) {
        String string = graphComponent.getGraphControl()
                .getDisplayLabelForCell(state.getCell());
        graphComponent.getGraph().drawState(canvas, state, string);
