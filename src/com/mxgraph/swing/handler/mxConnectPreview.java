...
public class mxConnectPreview extends mxEventSource {
    ...
    public void revalidate(mxCellState state, Object cell) {
        mxGraph graph = graphComponent.getGraph();
        mxCellState tmp = graph.getView().getState(cell);
        tmp.setInvalid(true);
        graph.getView().validateBounds(state, cell);
        graph.getView().validatePoints(state, cell);
        mxIGraphModel mxIGraphModel2 = graph.getModel();
        int childCount = mxIGraphModel2.getChildCount(cell);
        for (int i = 0; i < childCount; i++) {
            revalidate(tmp, mxIGraphModel2.getChildAt(cell, i));
        }
    }
