...
public class mxGraphView extends mxEventSource {
    ...
    public void validate() {
        Object object = currentRoot != null ? currentRoot : graph.getModel().getRoot();
        if (object != null) {
            validateBounds(null, object);
            mxRectangle graphBounds = validatePoints(null, object);
            if (graphBounds == null) {
        ...
        /*mxRectangle graphBounds = getBoundingBox(
                validateCellState(validateCell((currentRoot != null) ? currentRoot
                : graph.getModel().getRoot())));
        setGraphBounds((graphBounds != null) ? graphBounds : new mxRectangle());*/
    }

    public void validateBounds(mxCellState state, Object object) {
        mxIGraphModel model = graph.getModel();
        mxCellState mcs = getState(object, true);
        if (mcs != null && mcs.isInvalid()) {
            if (!graph.isCellVisible(object)) {
                removeState(object);
            } else if (object != currentRoot && state != null) {
                mcs.getAbsoluteOffset().setX(0);
                mcs.getAbsoluteOffset().setY(0);
                mcs.setOrigin(new mxPoint(state.getOrigin()));
                mxPoint geo = graph.getCellGeometry(object);
                if (geo != null) {
                    if (!model.isEdge(object)) {
                        mxPoint origin = mcs.getOrigin();
                        mxPoint offset = ((mxGeometry) geo).getOffset();
                        if (offset == null) {
                            offset = EMPTY_POINT;
                        }
                        if (((mxGeometry) geo).isRelative()) {
                            origin.setX(origin.getX() + geo.getX()
                                    * state.getWidth() / scale + offset.getX());
                            origin.setY(origin.getY() + geo.getY()
                                    * state.getHeight() / scale + offset.getY());
                        } else {
                            mcs.setAbsoluteOffset(
                                    new mxPoint(scale * offset.getX(),
                                            scale * offset.getY()));
                            origin.setX(origin.getX() + geo.getX());
                            origin.setY(origin.getY() + geo.getY());
                        }
                    ...
                    mcs.setX(scale
                            * (translate.getX() + mcs.getOrigin().getX()));
                    mcs.setY(scale
                            * (translate.getY() + mcs.getOrigin().getY()));
                    mcs.setWidth(scale * ((mxRectangle) geo).getWidth());
                    mcs.setHeight(scale * ((mxRectangle) geo).getHeight());
                    if (model.isVertex(object)) {
                        updateVertexLabelOffset(mcs);
                    }
                }
            }
            mxPoint childOffset = graph.getChildOffsetForCell(object);
            if (childOffset != null) {
                mcs.getOrigin().setX(
                        mcs.getOrigin().getX() + childOffset.getX());
                mcs.getOrigin().setY(
                        mcs.getOrigin().getY() + childOffset.getY());
            }
        }
        if (!(mcs == null
                || graph.isCellCollapsed(object) && object != currentRoot)) {
            int n = model.getChildCount(object);
            for (int i = 0; i < n; i++) {
                validateBounds(mcs, model.getChildAt(object, i));
            }
        }
    }

    public mxRectangle validatePoints(mxCellState state, Object cell) {
        mxRectangle mrect;
        mxIGraphModel model = graph.getModel();
        mxCellState mState = getState(cell);
        mxRectangle rect = null;
        if (mState != null) {
            if (mState.isInvalid()) {
                mxGeometry geo = graph.getCellGeometry(cell);
                mxPoint pt = getPoint(state, geo);
                if (geo != null && model.isEdge(cell)) {
                    mxCellState mcs = getState(getVisibleTerminal(cell, true));
                    if (mcs != null
                            && model.isEdge(mcs.getCell())
                            && !model.isAncestor(mcs, cell)) {
                        mxCellState parent = getState(
                                model.getParent(mcs.getCell()));
                        validatePoints(parent, mcs);
                    }
                    mxCellState target = getState(getVisibleTerminal(cell, false));
                    if (target != null
                            && model.isEdge(target.getCell())
                            && !model.isAncestor(target, cell)) {
                        mrect = getState(model.getParent(target.getCell()));
                        validatePoints((mxCellState) mrect, target);
                    }
                    updateFixedTerminalPoints(
                            mState, mcs, target);
                    updatePoints(
                            mState, geo.getPoints(),
                            mcs, target);
                    updateFloatingTerminalPoints(mState, mcs, target);
                    updateEdgeBounds(mState);
                    mState.setAbsoluteOffset(getPoint(mState, geo));
                } else if (geo != null && geo.isRelative()
                        && state != null &&
                        model.isEdge(state.getCell())
                        && pt != null) {
                    mState.setX(pt.getX());
                    mState.setY(pt.getY());
                    pt.setX(pt.getX() / scale - translate.getX());
                    pt.setY(pt.getY() / scale - translate.getY());
                    mState.setOrigin(pt);
                    childMoved(state, mState);
                }
                mState.setInvalid(false);
            }
            if (model.isEdge(cell) || model.isVertex(cell)) {
                updateLabelBounds(mState);
                rect = new mxRectangle(updateBoundingBox(mState));
            }
        }
        if (!(mState == null || graph.isCellCollapsed(cell) && cell != currentRoot)) {
            int childCount = model.getChildCount(cell);
            for (int i = 0; i < childCount; i++) {
                Object child = model.getChildAt(cell, i);
                mrect = validatePoints(mState, child);
                if (mrect != null) {
                    if (rect == null) {
                        rect = mrect;
                    } else {
    ...
    protected void childMoved(mxCellState mcs, mxCellState state) {
    ...
    public void updateLabelBounds(mxCellState state) {
        ...
        if (mxUtils.getString(style, mxConstants.STYLE_OVERFLOW, "").equals("fill")) {
            ...
            mxCellState edge = !graph.getModel().isEdge(cell)
                    ? state : null;
            state.setLabelBounds(
                    mxUtils.getLabelPaintBounds(label, style, graph.isHtmlLabel(cell),
                            state.getAbsoluteOffset(), edge, scale));
        }
    ...
    public mxPoint getNextPoint(mxCellState edge, mxCellState opposite,
        ...
        // todo 源码和IDEA为pts.size() >= 2;cfr,jd不同
            ...
            if (p0 == null || pe == null) {
                ...
                if (pt != null) {
                    ...
                    for (int i = 1; i < points.size(); ++i) {
                        ...
                        if (tmp != null) {
                            double d8 = pt.getX() - tmp.getX();
                            double d9 = pt.getY() - tmp.getY();

                            double segment = Math.sqrt(d8 * d8 + d9 * d9);
